...
from steam_review_ml.igdb.constants import IGDB_GAMES_LOOKUP_PARQUET, PIPELINE_DEFAULT_GAME_FIELD_NAMES
...
def test_from_json_uses_defaults_and_resolves_paths(tmp_path: Path) -> None:
    ...
    assert job_config.output_dir == repo_root / "artifacts/igdb"
    assert job_config.output_filename == IGDB_GAMES_LOOKUP_PARQUET
...
def test_from_json_parses_embed_text_fields(tmp_path: Path) -> None:
    job_config = IgdbGamesJobConfig.from_json(
        tmp_path,
        {"output_dir": "artifacts/igdb", "embed_text_fields": ["summary", "storyline"]},
    )
    assert job_config.embed_text_fields == ("summary", "storyline")


def test_from_json_parses_taxonomy_fields(tmp_path: Path) -> None:
    job_config = IgdbGamesJobConfig.from_json(
        tmp_path,
        {
            "output_dir": "artifacts/igdb",
            "taxonomy_fields": ["genres", "themes"],
        },
    )
    assert job_config.taxonomy_fields == ("genres", "themes")


def test_resolved_game_fields_pipeline_preset() -> None:
    job_config = IgdbGamesJobConfig(
        repo_root=Path("/repo"),
        output_dir=Path("/repo/artifacts/igdb"),
    )
