...
4. (v2 metadata **Job 1**) `python scripts/recs_job_igdb_games.py configs/recs_job_igdb_games.json` — output under `artifacts/igdb/` (config `output_dir`). Requires `TWITCH_CLIENT_ID` / `TWITCH_CLIENT_SECRET` in repo-root `.env` when `skip_fetch` is false. Fetches catalog-scoped games (~315 Steam titles + manual mock in `configs/igdb_steam_mock_rows.json`) into `lookups/games.parquet`, then fetches taxonomy entity tables (`genres`, `themes`, `keywords`, `game_modes`, `player_perspectives`) into `lookups/*.parquet`. Embeds `embed_text_fields` (default `summary`, `storyline`) on games rows and `name` on each taxonomy lookup (`name__use`). Writes `igdb_join_report.json`, `meta.json`, `lookups/lookup_meta.json`. Set `skip_fetch: true` to re-embed from existing lookups without API calls. Config: [`configs/recs_job_igdb_games.json`](../configs/recs_job_igdb_games.json).
5. (v2 metadata **Job 2**) `python scripts/recs_job_igdb_games_enriched.py configs/recs_job_igdb_games_enriched.json` — reads `artifacts/igdb/lookups/`, writes `artifacts/igdb/igdb_games__enriched.parquet`. No API / no TensorFlow. Joins taxonomy lookups onto games by **FK id** → `{field}_names` and `{field}_names__use` parallel arrays. Config: [`configs/recs_job_igdb_games_enriched.json`](../configs/recs_job_igdb_games_enriched.json). EDA: join coverage [`igdb_001`](../notebooks/igdb/igdb_001_eda_join_coverage.ipynb), per-field profiles [`igdb_002`](../notebooks/igdb/igdb_002_eda_game_review.ipynb), taxonomy FK resolution [`igdb_003`](../notebooks/igdb/igdb_003_eda_taxonomy_resolution.ipynb).
6. (RAG extension, in progress — see [`plans/rag_extension_plan.md`](plans/rag_extension_plan.md)) `python scripts/recs_job_game_chunks.py configs/recs_job_game_chunks.json` then `pip install -e '.[rag]'` and `python scripts/recs_job_game_chunk_embeddings.py configs/recs_job_game_chunk_embeddings.json` → `artifacts/recs/embeddings/game_chunks/chroma/`.

### RAG extension — chunking + embedding (Stages 1–2, in progress)

Prototype jobs for [`docs/plans/rag_extension_plan.md`](plans/rag_extension_plan.md); not yet wired into the shipped serve stack or `recs_job_eval_offline.py`. Requires the train split parquet (step 4) and `artifacts/igdb/igdb_games__enriched.parquet` (v2 metadata Job 2 above).
...
11a. (Optional, in progress) RAG extension Stages 1–2: `recs_job_game_chunks.py` then `recs_job_game_chunk_embeddings.py` (requires `.[rag]`) → `artifacts/recs/embeddings/game_chunks/chroma/`. Not yet consumed by eval or serving.
