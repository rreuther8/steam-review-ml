...
- **Built**: `src/steam_review_ml/evaluation/llm_judge.py` — scores each cached explanation on two 1-5 axes, **faithfulness** (is every claim grounded in the two texts given, or invented) and **relevance** (does the explanation tie something specific about the query game to something specific about the recommendation, vs. generic boilerplate), judged **game-vs-game**: both texts are IGDB metadata (query game + rec game, via `build_candidate_text_lookup`), never the user's raw review — matching exactly what `generate_explanation()` was grounded in. Judging against the review instead would false-flag genuinely-present query-game details as "invented" whenever that particular review didn't happen to mention them, and would make relevance a function of how descriptive the review was rather than of explanation quality (same distinction `explanation_eval_pipeline.py` draws between `relevance_cosine` and `relevance_cosine_query_game`). Judge model is Claude (Anthropic API, default `claude-haiku-4-5-20251001`) — deliberately a *different* model family than the local Llama-3.1-8B that generated the explanations, to avoid a model grading its own output.
...
- **Done — first calibration pass (n=10, 2026-08-31)**: hand-labeled the seeded 10-example sample, ran the judge (Haiku, ~6.4K input/929 output tokens, negligible cost) on the same 10, `--compare`d. Judge relevance tracked the human labels reasonably (Spearman 0.66) but with weak absolute agreement (20% exact, 40% within-1); judge faithfulness showed ~no correlation (0.12), though human faithfulness labels were heavily skewed (8/10 rated 5), which alone destabilizes a Spearman estimate at this n. The `relevance_cosine_query_game` heuristic fix from the same session outperformed the old review-based `relevance_cosine` against human judgment (0.34 vs. -0.16 Spearman) — concrete evidence the disconnect-fix helped, independent of the judge's own reliability. See [`ideal_state_roadmap.md`](../plans/ideal_state_roadmap.md) § 2.5.
- **Todo: expand the calibration sample.** n=10 is too small and too skewed (faithfulness labels almost all 5s) to trust any of the above numbers yet — in particular there's barely any labeled *low*-faithfulness case for the judge to get right or wrong against. Raise `n_sample` (calibration config) and `limit` (judge config) together, e.g. to 30-50, hand-label the larger set (bump `seed`/reuse the same one to extend rather than resample from scratch if avoiding overlap matters), rerun the judge (still cheap: ~650 input tokens/example observed) and `--compare` again before drawing real conclusions about judge reliability.
