...
**Notebook (text profiles):** `notebooks/models/recs/recs_001_game_profiles.ipynb` writes `artifacts/recs/game_profile.parquet` (`profile_text` column; vectors in a later step).
...
**Tabular baselines and simple linear models** already exist under `notebooks/models/tabular/` (`model_000_*`, `model_001_*`, `model_002_*`). They **do not** satisfy the items below; the **north star** remains **preference extraction + content retrieval + @K** for recommendations. **Game profiles:** `notebooks/models/recs/recs_001_game_profiles.ipynb` → `artifacts/recs/game_profile.parquet`.
...
3. Implement similarity retrieval and top‑N recommendation output (structured query → embed → game profiles).
